...
read_hh <- function(path, vars) {
  read_sav(path) %>% dplyr::select(any_of(vars))
}

read_ch <- function(path, vars) {
  read_sav(path) %>% dplyr::select(any_of(vars))
}
...
vars_ch <- c("HH1","HH2","CA1")

# MICS6
hh6 <- read_hh("E:/regression-analysis/final-exam/Viet Nam MICS6 Datasets/Viet Nam MICS6 SPSS Datasets/hh.sav", vars_hh_ms6)
ch6 <- read_ch("E:/regression-analysis/final-exam/Viet Nam MICS6 Datasets/Viet Nam MICS6 SPSS Datasets/ch.sav", vars_ch)

# MICS5
hh5 <- read_hh("E:/regression-analysis/final-exam/Regression-Analysis/final-exam/Viet Nam_MICS5_Datasets/Viet Nam MICS 2013-14 SPSS Datasets/hh.sav", vars_hh_ms5)
ch5 <- read_ch("E:/regression-analysis/final-exam/Regression-Analysis/final-exam/Viet Nam_MICS5_Datasets/Viet Nam MICS 2013-14 SPSS Datasets/ch.sav", vars_ch)

# MICS4
hh4 <- read_hh("E:/regression-analysis/final-exam/Regression-Analysis/final-exam/Vietnam_MICS4_Datasets/Vietnam MICS 2010-2011 SPSS Datasets/hh.sav", vars_hh_ms4)
ch4 <- read_ch("E:/regression-analysis/final-exam/Regression-Analysis/final-exam/Vietnam_MICS4_Datasets/Vietnam MICS 2010-2011 SPSS Datasets/ch.sav", vars_ch)

# MICS3
hh3 <- read_hh("E:/regression-analysis/final-exam/Regression-Analysis/final-exam/Vietnam_Datasets/Vietnam MICS 2006 SPSS Datasets/hh.sav", vars_hh_ms3)
ch3 <- read_ch("E:/regression-analysis/final-exam/Regression-Analysis/final-exam/Vietnam_Datasets/Vietnam MICS 2006 SPSS Datasets/ch.sav", c("diaban","hh2","ca1"))

# MICS2
hh2 <- read_hh("E:/regression-analysis/final-exam/Regression-Analysis/final-exam/Viet Nam 2000 MICS_Datasets/hhVI.sav", vars_hh_ms2)
ch2 <- read_ch("E:/regression-analysis/final-exam/Regression-Analysis/final-exam/Viet Nam 2000 MICS_Datasets/chVI.sav", c("HI1","HI2","CI1"))
```
...
-   Thay dữ liệu thiếu bằng cách sử dụng thuật toán **KNN** để dự đoán giá trị của dữ liệu thiếu thông qua các dữ liệu xung quang dữ liệu thiếu.

-   Giữ nguyên giá trị trong `windex5r, windex5u`.

```{r op-missing, results = 'hide'}
# --- KNN Imputation (chỉ cho numeric, trừ windex5r & windex5u) ---
hh_temp <- hh6 %>% dplyr::select(-c(windex5r, windex5u))
hh_knn  <- VIM::kNN(hh_temp, k = 5) %>% dplyr::select(-matches("_imp$"))

# Ghép lại với 2 biến đặc thù
hh6_clean <- bind_cols(
  hh_knn,
  hh6 %>% dplyr::select(any_of(c("windex5r", "windex5u")))
)

# Kiểm tra lại
colMeans(is.na(hh6_clean))*100
```
...
**Nhận xét:** Tỷ lệ hộ có nhà tiêu hợp vệ sinh là 79%, vẫn còn 21% hộ chưa có nhà tiêu đạt chuẩn, tiềm ẩn nguy cơ ô nhiễm.
...
**Nhận xét:** Khu vực thành thị có điều kiện vệ sinh tốt hơn nhiều so với nông thôn. Tỷ lệ hộ có nhà tiêu hợp vệ sinh ở nông thôn thấp hơn 19.2% so với thành thị.
...
-   `windex5u`, `HC2` có ý nghĩa thống kê (p \< 0.001).
...
# SỬA LỖI: Đảm bảo list các năm có đúng 5 phần tử
